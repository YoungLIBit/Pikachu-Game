...
class Game extends React.Component {
  ...
  reloadHandler = () => {
    if (this.state.reload <= 0) {
      if (window.confirm("You lost! Restart game?")) {
        this.renew();
      }
    } else {
  ...
  onTimeout = () => {
    if (window.confirm("You lost cuz timeout! Restart game?")) {
      this.renew();
    }
  };
  ...
  checkRectY = (p1, p2) => {
    ...
    for (let xi = pup.x + 1; xi < pdown.x; xi++) {
      if (
        this.checkLineY(pup.x, xi, pup.y) &&
        this.checkLineX(pup.y, pdown.y, xi) &&
        this.checkLineY(xi, pdown.x, pdown.y) &&
        this.state.items[xi][pup.y] === 0 &&
        this.state.items[xi][pdown.y] === 0
      ) {
        if (pup.y > pdown.y) {
          this.lines.push(
  ...
  checkEdge = (p1, p2) => {
    ...
    if (this.state.items[p.x][p.y] === 0) {
      ...
      if (
        ...
        if (pleft.x > pright.x) {
          this.lines.push({ x: p.x, y: p.y, value: "bottom_right" });
        } else {
          this.lines.push({ x: p.x, y: p.y, value: "top_right" });
        }
        return true;
      }
    }
    ...
    if (
      ...
      if (pleft.x > pright.x) {
        this.lines.push({ x: p.x, y: p.y, value: "top_left" });
      } else {
        this.lines.push({ x: p.x, y: p.y, value: "bottom_left" });
      }
      return true;
    }
  ...
  checkExtendY = (p1, p2, maxX) => {
    ...
    for (let xi = pdown.x + 1; xi <= maxX + 1; xi++) {
      ...
      if (
        this.checkLineY(pup.x, xi, pup.y) &&
        this.checkLineY(pdown.x, xi, pdown.y) &&
        this.checkLineX(pup.y, pdown.y, xi) &&
        this.state.items[xi][pup.y] === 0 &&
        this.state.items[xi][pdown.y] === 0
      ) {
  ...
  isPair = (p1, p2) => {
    ...
    if (
      this.state.items[x1][y1] !== this.state.items[x2][y2] ||
      (x1 === x2 && y1 === y2)
    ) {
      return false;
    }
  ...
  constructor(props) {
    ...
    this.listScore = localStorage.getItem("listScore")
      ? JSON.parse(localStorage.getItem("listScore"))
      : new Array(5);
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.doneLine) {
      ...
      this.newItems[this.state.square1.x][this.state.square1.y] = this.newItems[
    ...
    if (this.state.square1 && this.state.square2) {
      ...
      for (i = 0; i < this.satisfiableItems[value].length; i++) {
        // compare two object
        if (
          (this.satisfiableItems[value][i].square1.x === this.state.square1.x &&
            this.satisfiableItems[value][i].square1.y ===
              this.state.square1.y &&
            this.satisfiableItems[value][i].square2.x ===
              this.state.square2.x &&
            this.satisfiableItems[value][i].square2.y ===
              this.state.square2.y) ||
          (this.satisfiableItems[value][i].square2.x === this.state.square1.x &&
            this.satisfiableItems[value][i].square2.y ===
              this.state.square1.y &&
            this.satisfiableItems[value][i].square1.x ===
              this.state.square2.x &&
          ...
          if (this.lastLines.length > 0) {
            this.lastLines.map(
              (line) => (this.newItems[line.x][line.y] = line.value)
            );
          }
          ...
          this.listPosItem[value][this.satisfiableItems[value][i].item1] =
            this.listPosItem[value][this.listPosItem[value].length - 1];
          this.listPosItem[value].pop();
          this.listPosItem[value][this.satisfiableItems[value][i].item2] =
            this.listPosItem[value][this.listPosItem[value].length - 1];
          this.listPosItem[value].pop();

          // Remove couple from satisfiableItems array
          this.satisfiableItems[value][i] =
            this.satisfiableItems[value][
              this.satisfiableItems[value].length - 1
            ];
  ...
  isExist() {
    ...
    // Initialize 2d array
    this.satisfiableItems = [...Array(this.amount + 1)]
      .fill(null)
      .map(() => []);
      ...
      for (j = 0; j < this.listPosItem[i].length; j++) {
        for (k = j + 1; k < this.listPosItem[i].length; k++) {
          ...
          if (this.isPair(this.listPosItem[i][j], this.listPosItem[i][k])) {
            this.satisfiableItems[i].push({
              square1: this.listPosItem[i][j],
              square2: this.listPosItem[i][k],
              lines: this.lines,
              item1: j,
              item2: k,
            });
  ...
  saveScore(score) {
    if (
      score > this.listScore[this.listScoreLength - 1] ||
      this.listScore.length < this.listScoreLength
    ) {
      this.listScore.push(score);
    }
    localStorage.setItem(
      "listScore",
      this.listScore.sort((a, b) => b - a)
    );
  }
  ...
  handleLevel(level) {
    switch (level) {
      ...
      case 6:
        this.newItems = move3CenterLeftRight(
          this.newItems,
          this.state.square1.x,
          this.state.square1.y
        );
        this.newItems = move3CenterLeftRight(
          this.newItems,
      ...
      case 7:
        ...
        this.newItems = move3CenterTopDown(
          this.newItems,
          this.state.square1.y,
          this.state.square1.x
        );
      ...
      case 8:
        ...
        this.newItems = move3OutLeftRight(
          this.newItems,
          this.state.square1.x,
          this.state.square1.y
        );
      ...
      <div className="game">
        <div className="game-board">
          <Timer
            ...
            onFinishInterval={this.onTimeout}
